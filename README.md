...
采用 **Pages Git 集成**（非 Wrangler 直传），Astro 保持静态输出，不安装 Cloudflare adapter。

- **生产分支**：`main`，**根目录**：仓库根
- **构建命令**：`npm run build:content && npm run check:functions`
- **输出目录**：`dist`
- **Function compatibility date**：`2026-07-13`
- **加密 secret**：`NOTION_TOKEN`
- **变量**：`NOTION_DATA_SOURCE_ID`、`SITE`（正式域名）、`NODE_VERSION=22.12.0`

仅 `/notion-webhook` 这一条路由运行 Pages Function（`functions/notion-webhook.ts`），用于验证 Notion webhook 签名并触发 Deploy Hook；它不参与页面渲染。Function 类型契约由 `npm run generate:function-types` 生成并提交为 `functions/types.d.ts`。

### 自动发布：Notion webhook → Deploy Hook

1. 在 Pages 项目 *Settings → Builds & deployments → Deploy hooks* 创建一个 **`main` 分支** 的 `notion-content` Deploy Hook，把 URL 存为加密变量 `CLOUDFLARE_DEPLOY_HOOK_URL`。

Function 只接受四类事件并按 data source ID 过滤后 POST 到该 Hook：
...
> subscription URL 不入库；验证后不重建 subscription。错误的 `setup` nonce 或过期时间不会产生日志；伪造签名返回 401 且不部署。Hook 返回 2xx → 202；网络/408/429/5xx → 502；其它 4xx → 204 且不部署。
